...
const AgentTestScene = () => {
    const frame = (0, remotion_1.useCurrentFrame)();
    const { fps } = (0, remotion_1.useVideoConfig)();
    // Header entrance
    const headerSpring = (0, remotion_1.spring)({
        frame,
        fps,
        config: { damping: 200 },
    });
    const headerOpacity = (0, remotion_1.interpolate)(headerSpring, [0, 1], [0, 1]);
    // Editor entrance
    const editorDelay = 0.3 * fps;
    const editorSpring = (0, remotion_1.spring)({
        frame,
        fps,
        delay: editorDelay,
        config: { damping: 200 },
    });
    const editorOpacity = (0, remotion_1.interpolate)(editorSpring, [0, 1], [0, 1]);
    const editorY = (0, remotion_1.interpolate)(editorSpring, [0, 1], [30, 0]);
    // Print insertion springs (staggered)
    const baseDelay = 1.0 * fps;
    const printSprings = PRINT_INSERTIONS.map((ins) => (0, remotion_1.spring)({
        frame,
        fps,
    ...
    for (let i = 0; i < CODE_LINES.length; i++) {
        ...
        const insertions = PRINT_INSERTIONS.map((ins, idx) => ({ ...ins, idx })).filter((ins) => ins.afterLine === i);
    ...
    <span style={{ fontSize: 50 }}>🤖</span>
            ...
            return renderLines.map((line, idx) => {
                if (line.isDebug) {
                    const h = (0, remotion_1.interpolate)(line.springVal, [0, 1], [0, 38]);
                    const op = (0, remotion_1.interpolate)(line.springVal, [0, 1], [0, 1]);
                    ...
                    return (<div key={idx} style={{ display: "flex", gap: 14 }}>
											<span style={{
                            color: "#484F58",
                            minWidth: 28,
                            textAlign: "right",
...
exports.AgentTestScene = AgentTestScene;
//# sourceMappingURL=AgentTestScene.js.map